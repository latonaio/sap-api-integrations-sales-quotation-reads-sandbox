...
func (c *SAPAPICaller) callSalesQuotationSrvAPIRequirementHeader(api, salesQuotation string) ([]sap_api_output_formatter.Header, error) {
	...
	c.getQueryWithHeader(req, salesQuotation)
...
func (c *SAPAPICaller) callSalesQuotationSrvAPIRequirementItem(api, salesQuotation, salesQuotationItem string) ([]sap_api_output_formatter.Item, error) {
	...
	c.setHeaderAPIKeyAccept(req)
	c.getQueryWithItem(req, salesQuotation, salesQuotationItem)
...
func (c *SAPAPICaller) getQueryWithHeader(req *http.Request, salesQuotation string) {
	params := req.URL.Query()
	params.Add("$filter", fmt.Sprintf("SalesQuotation eq '%s'", salesQuotation))
	req.URL.RawQuery = params.Encode()
}

func (c *SAPAPICaller) getQueryWithItem(req *http.Request, salesQuotation, salesQuotationItem string) {
	params := req.URL.Query()
	params.Add("$filter", fmt.Sprintf("SalesQuotation eq '%s' and SalesQuotationItem eq '%s'", salesQuotation, salesQuotationItem))
	req.URL.RawQuery = params.Encode()
}
